...
const ms = {
  ...
  box: { background: 'var(--color-surface)', border: '2px solid var(--color-ink)', borderRadius: 8, width: '100%', maxWidth: 400, padding: 26, boxShadow: 'var(--shadow-modal)' },
...
export default function EditUserModal() {
  ...
  const userId = activeModal?.data?.userId || currentUserId;
  ...
  const handleSubmit = () => {
    ...
    updateUser(user.id, {
      name: name.trim(),
      desc: desc.trim() || 'Film enthusiast',
      emoji,
      avatarImg,
    });
  };
  ...
  return (
    <div className="modal-overlay" style={ms.overlay} onClick={e => e.target === e.currentTarget && closeModal()}>
      <div className="modal-content" style={ms.box}>
        <div style={ms.eyebrow}>Viewer Profile</div>
        <h2 style={ms.title}>Edit Profile</h2>

        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
          <ProfileAvatar user={{ emoji, avatarImg }} size={56} />
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <input
            ...
            style={{ width: '100%', padding: '8px 11px', borderRadius: 4, fontFamily: 'var(--font-body)', fontSize: '.82rem' }}
            maxLength={20}
            value={name}
        ...
        <div style={{ marginBottom: 18 }}>
          <label style={ms.label}>Fallback Emoji</label>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button className="btn-primary" style={{ flex: 1, padding: '9px 0', fontFamily: 'var(--font-mono)', fontSize: '.6rem', letterSpacing: '.1em', textTransform: 'uppercase', borderRadius: 4, cursor: 'pointer' }} onClick={handleSubmit}>Save</button>
